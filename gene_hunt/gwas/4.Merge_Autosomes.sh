...
~/modules/plink2 --threads 20 --vcf autosomal_files/autos.vcf.gz --allow-extra-chr --set-missing-var-ids @:# \
        --rm-dup --indep-pairwise 50 5 0.1 --maf 0.05 --hwe 1e-10 --max-alleles 2 --min-alleles 2 --out autosomal_files/autos_canorus_LD
        
#extract, also a vcf and run PCA 
~/modules/plink2 --threads 20 --vcf autosomal_files/autos.vcf.gz --allow-extra-chr --set-missing-var-ids @:# \
        --extract autosomal_files/autos_canorus_LD.prune.in \
        --make-bed --recode vcf bgz --pca --out autosomal_files/autos_canorus_LD
bcftools index --threads 20 autosomal_files/autos_canorus_LD.vcf.gz
sed -i 's/chr_//g' autosomal_files/autos_canorus_LD.bim
